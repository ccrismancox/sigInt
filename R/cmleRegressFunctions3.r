...
dpR <- rbind((((param$VA*pR*exPR + (-param$SA + param$VA*(-pR + 1))*exPR)*P1*D2 + (param$VA*pR*exPR + (-param$SA + param$VA*(-pR + 1))*exPR)*P2*D1)/(P1P2)),
                     # ((-pR**2*pC*exPR + (-pR + 1)*(-(param$VA*pR*exPR + (-param$SA + param$VA*(-pR + 1))*exPR)*P1*D2 - (param$VA*pR*exPR + (-param$SA + param$VA*(-pR + 1))*exPR)*P2*D1))/(pC*(-pR + 1))),
                     exPR * (1 + ((D2*P1 + D1*P2)*(param$SA - param$VA))/pC + 1/(-1 + pR) + pR),
                     # ((1 + exPR)*(pR**2*PBdv*exPR + pR*(-param$VA*pR*exPR + (param$SA - param$VA*(-pR + 1))*exPR)*Del_v1d1)/PBdv),
